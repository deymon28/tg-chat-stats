...
CONFIG = {
    ...
    "output_folder": "telegram_report_advanced_3",
    "language": "ukrainian",
    "report_language": "en",
    "font_path": r'C:\PROJECTS\Python_scripts\fonts\segoeui.ttf',
    "generate_pdf": True,
    "wkhtmltopdf_path": r"C:\Apps\wkhtmltopdf\bin\wkhtmltopdf.exe"
}

# --- LOCALIZATION STRINGS (omitted for brevity) ---
UI_STRINGS = {
    'uk': {
...
# --- Function definitions (add_readability, get_sentiment, etc. - omitted for brevity) ---
...
# --- NEW: Helper function to remove emojis from labels to prevent font errors ---
...
def create_visualizations(stats, output_dir, font_path, loc):
    ...
    author_counts_clean = stats['author_counts'].copy()
    author_counts_clean.index = author_counts_clean.index.map(remove_emojis)
    plt.figure(figsize=(12, 8))
    sns.barplot(y=author_counts_clean.index, x=author_counts_clean.values)
    ...
    if stats.get('emoji_counts'):
        ...
        try:
            emoji_font_prop = matplotlib.font_manager.FontProperties(fname=emoji_font_path)
        except:
            emoji_font_prop = font_prop

        plt.figure(figsize=(12, 8))
        sns.barplot(y='emoji', x='count', data=emoji_df)
        plt.yticks(fontproperties=emoji_font_prop)
    ...
    if 'reply_matrix' in stats and not stats['reply_matrix'].empty:
        reply_matrix_clean = stats['reply_matrix'].copy()
        reply_matrix_clean.index = reply_matrix_clean.index.map(remove_emojis)
        reply_matrix_clean.columns = reply_matrix_clean.columns.map(remove_emojis)
        plt.figure(figsize=(14, 10))
        sns.heatmap(reply_matrix_clean, cmap="BuPu", annot=True, fmt=".0f", linewidths=.5)
    ...
    if 'lexical_diversity' in stats and not stats['lexical_diversity'].empty:
        lexical_diversity_clean = stats['lexical_diversity'].copy()
        lexical_diversity_clean.index = lexical_diversity_clean.index.map(remove_emojis)
        plt.figure(figsize=(12, 8))
        lexical_diversity_clean.sort_values().plot(kind='barh')
    ...
    if 'night_activity' in stats and not stats['night_activity'].empty:
        night_activity_clean = stats['night_activity'].copy()
        night_activity_clean.index = night_activity_clean.index.map(remove_emojis)
        plt.figure(figsize=(12, 8))
        night_activity_clean.head(15).sort_values().plot(kind='barh')
    ...
    if stats.get('top_bigrams'):
        ...
        plt.figure(figsize=(12, 8))
        # --- FIXED: Updated call to remove FutureWarning ---
        sns.barplot(x='count', y='phrase', data=bigram_df, hue='phrase', palette='mako', legend=False)
...
def generate_html_report(stats, output_dir, loc):
    ...
    def series_to_html_table(data, headers):
        # --- FIXED: Robust check for both lists and pandas Series ---
        if data is None: return f"<p>No data available.</p>"
        # Check for pandas objects with .empty, otherwise check truthiness for lists/dicts
        ...
        for index, value in items:
            # Clean emojis from sticker pack names for display
            clean_index = remove_emojis(str(index))
            html_content += f"<tr><td>{clean_index}</td><td>{value}</td></tr>"
        return html_content + "</table>"

    def df_to_html_table(df):
        if df is None or df.empty: return f"<p>No data available.</p>"
        # Clean emojis from user names (the index) for display
        df_clean = df.copy()
        df_clean.index = df_clean.index.map(remove_emojis)
        return df_clean.to_html(classes='data-table', index=True)

    # HTML content building (omitted for brevity, no changes from previous version)
    html_template = f"""
